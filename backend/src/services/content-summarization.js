...
class ContentSummarizationService {
    ...
    try {
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media expert who creates engaging and concise content.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async generateSummaryVariants(content, platform = SocialPlatform.TWITTER, count = 3) {
    try {
      const summaries = [];
